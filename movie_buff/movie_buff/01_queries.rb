...
def biggest_cast
  ...
  #Why can't we group by castings#movie_id
  Movie.joins(:castings).group('movies.id').select(:id, :title).order("COUNT(*) DESC").limit(3)
  # Movie.joins(:castings).group('castings.movie_id').select(:id, :title).order("COUNT(*) DESC").limit(3)
end
...
def directed_by_one_of(them)
  ...
  #why does includes also include director id in addition to movie id?
  # Movie.select(:id, :title).includes(:director).where(actors: {name: them})
